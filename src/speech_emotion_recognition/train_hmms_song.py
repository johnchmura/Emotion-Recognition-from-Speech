...
classes = sorted(df["Vocal Channel"].unique())
label_map = {vc: i for i, vc in enumerate(classes)}
print("Vocal Channel classes:", classes)

all_actors = df["Actor"].unique().tolist()
...
print(f"Train utterances: {len(train_df)}, Test utterances: {len(test_df)}")
...
for _, row in train_df.iterrows():
    ...
    train_labels.append(label_map[row["Vocal Channel"]])

models, histories = {}, {}
for vc_idx, vc in enumerate(classes):
    seqs = [s for s, l in zip(train_seqs, train_labels) if l == vc_idx]
    lengths = [s.shape[0] for s in seqs]
    X_cat = np.vstack(seqs)
    print(f"Training HMM for '{vc}' ({len(seqs)} samples)…")
    m = hmm.GaussianHMM(
    ...
    m.fit(X_cat, lengths)
    models[vc_idx] = m
    histories[vc_idx] = m.monitor_.history
...
disp.plot()
plt.title("HMM Vocal Channel Confusion Matrix")
...
plt.figure()
for vc_idx, vc in enumerate(classes):
    plt.plot(histories[vc_idx], label=vc)
plt.title("HMM EM Log-Likelihood per Iteration (Vocal Channel)")
